'''
The clay daemon code, used to watch the pool directory and set up the automatic
deployment pool of vm images.
'''
...
class Daemon(object):
    ...
    def __stamp_loc(self):
        ...
        '''
        stamp = os.path.join(self.opts['pool'], '.clayd_stamp.p')
    ...
    def _check_stamp(self):
        ...
        again. This ensures that only one clay daemon is watching files at a
        time.
        '''
        wait = True
    ...
    def _set_stamp(self):
        '''
        Saves the run stamp used to ensure that clayd is only running on one
